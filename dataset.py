...
class CustomDirDataset(Dataset):
    """
    Dataset that loads images from a directory structure (images-only version).
    """
    def __init__(self, data_dir):
        PIL.Image.init()
        supported_ext = PIL.Image.EXTENSION.keys() | {'.npy'}
...
class CustomDirDatasetWithFeatures(Dataset):
    """
    Dataset that loads images and pre-computed features from directory structures.
    """
    def __init__(self, data_dir, vae_latents_name="vae-sd"):
        PIL.Image.init()
        supported_ext = PIL.Image.EXTENSION.keys() | {'.npy'}

        self.images_dir = os.path.join(data_dir, 'images')
        self.features_dir = os.path.join(data_dir, vae_latents_name)
        ...
        # labels
        fname = 'dataset.json'
        with open(os.path.join(self.features_dir, fname), 'r') as f:
            labels = json.load(f)['labels']
        labels = dict(labels)
        labels = [labels[fname.replace('\\', '/')] for fname in self.feature_fnames]
        labels = np.array(labels)
